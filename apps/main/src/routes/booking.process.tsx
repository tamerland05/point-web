...
export const Route = createFileRoute("/booking/process")({
  ...
  loaderDeps: ({ search }) => ({ establishmentId: search.establishmentId }),
...
function RouteComponent() {
  ...
  const { establishmentId } = Route.useSearch()
  ...
  return (
    <>
      <BookingParityBackdrop enabled={parityCapture} variant="process" />
          ...
          footer={
            <div className="fixed inset-x-0 bottom-0 bg-[#efeff4] px-4 pt-3 pb-8">
              <BookingPrimaryButton
                onClick={() => {
                  void navigate({
                    search: { contactName: name.trim() || undefined, establishmentId },
                    to: "/booking/filled",
                  })
                }}
                variant="process"
